...
A component may expose properties, methods, state, or behaviour that make the underlying content easier to handle. It may provide values that can be read or changed, operations that can be performed, or state that can be tracked while the interface is active. This allows the rest of the Stack to work with meaningful objects instead of repeatedly handling raw representations or immutable structures.
...
Controls are responsible for turning interaction into meaningful interface behaviour. A control may update a value, trigger an operation, change state, request a new representation, or produce no change at all. The key distinction is that a control does not merely exist as part of the interface; it participates in the user’s ability to act upon the system.
...
This separates layouts from graphs. A layout arranges components according to a specific rule or pattern. A graph manages broader discovery, organisation, relationships, and lifecycle across a larger interface context. Layer 5 provides the layout objects themselves; Layer 4 determines how those objects participate in the larger frame of operation.
...
In top-down flow, Components, Controls, & Layouts receives interpreted structure and turns it into usable interface objects. The Stack has moved beyond knowing what the content means and now begins preparing that content for interaction, organisation, and eventual presentation.

At this stage, the interface can decide which parts of the interpreted content should become components, which components should be capable of handling interaction, and how those components should be arranged. The result is not yet a rendered frame or system output. It is an active interface model that can be organised by Layer 4 and eventually prepared for rendering.

### Role in Bottom-Up Flow

In bottom-up flow, Layer 5 is where routed interaction becomes meaningful behaviour. Lower layers may detect, translate, and route an action, but a control is where that action is finally understood in relation to the interface object it affects.
...
This relationship keeps the Model clean. Layer 5 defines the objects of the active interface. Layer 4 manages their larger organisation and prepares them for the lower layers of the Stack. A component should know how to represent and manage its own usable state; the graph and frame should know how that component fits into the wider interface flow.
